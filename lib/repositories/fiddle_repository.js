...
//mappings model=>row
var ModelToRowMapping = { 'id':'id', 'slug':'slug','user_id':'user_id','revision':'revision','fiddle':'fiddle','rom_hash':'rom_hash','created':'created' };
...
FiddleRepository.prototype.find = function(slug, revision) {
   ...
   return this.db.queryRows('select * from fiddles where slug = ? and revision = ?',[slug,revision])
       .then(function(result) {
           return (result && result.length > 0) ? rowAsFiddle(result[0]): null;
       });
};
